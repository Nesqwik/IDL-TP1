...
public class WatorFactory {
    ...
    private static FishAgent createNewFish(Environment env) {
        System.out.println("new fish");
        try {
            return (FishAgent) Class.forName(Config.getFishType()).getConstructor(Environment.class).newInstance(env);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    private static SharkAgent createNewShark(Environment env) {
        System.out.println("new shark");
        try {
            return (SharkAgent) Class.forName(Config.getSharkType()).getConstructor(Environment.class).newInstance(env);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
